		...
		function(){
			jQuery.validator.setDefaults({ 
				submitHandler : function(form) {
					if("function" == typeof(successBack)){
						successBack(form);
					} else {
						form.submit();
					}
			...
			var messages = {
				...
				equalTo: "两次输入不一致.",
				accept: "Please enter a value with a valid extension.",
				maxlength: $.validator.format("输入不能多于 {0} 个字符"),
			...
			$.extend($.validator.methods,{
				//Can only enter the number
				inputNumber: function(value, element, param) {
					if(element.value.replace(/[\d+]/ig,"").length>0){
				...
				isMobileOrPhone: function(value, element) {
					var length = value.length; 
					var tel = /^(\d{3,4}-?)?\d{7,9}$/g;
					if(this.optional(element) || tel.test(value) || (length == 11 && /^(((13[0-9]{1})|(15[0-9]{1})|(14[0-9]{1})|(17[0-9]{1})|(18[0-9]{1}))+\d{8})$/.test(value))){
						return true;
					} else {
						return false;
					}
