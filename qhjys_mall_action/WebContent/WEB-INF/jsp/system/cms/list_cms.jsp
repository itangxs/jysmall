...
<div class="membercontent">
	<!----------------左侧----------------------->
	<jsp:include page="/WEB-INF/jsp/system/cms/uLeft.jsp" flush="true" ><jsp:param name="val" value="4" /></jsp:include>
    <!--------------右侧------------------>
	<!--------------右侧------------------>
		<div class="memberright">
			<h3 style="padding-left: 10px; margin-bottom: 25px;">CMS列表</h3>
			<form id="from" action="/managermall/systemmall/cms/cmsList.do" method="post" class="zcform1">
			<input type="hidden" id="page" name="pageNum" value="${page.getPageNum()}">
				...
				<p class="clearfix" style="margin-bottom:10px;">
					...
					<select name="parentId">
		             ...
		             <option value="${cmsCategory1.id }"<c:if test="${parentId==cmsCategory1.id }"> selected="selected"</c:if>>${cmsCategory1.name }</option>
		             </c:forEach>
		            </select>
		        	...
		        	<option value="1"<c:if test="${enabled==0 }"> selected="selected"</c:if>>启用</option>
		        	</select>
		        	<label class="one" for="con-email4" >标题:</label>
        			<input class="con-email4" name="name" id="name" value="${name }"/>
			...
			<div class="member_myorder">
				<table width="100%" border="0" cellspacing="0" cellpadding="0">
					<tr>
						<th class="td160">名称</th>
						<th class="td160">类别</th>
					...
					<tr>
						...
						<td class="td160">${it.fatherName}</td>
						<td class="td160"><c:if test="${it.enabled==0}">不启用</c:if><c:if test="${it.enabled==1}">启用</c:if> </td>
						<td class="td160"><a href="/managermall/systemmall/cms/addCms.do?id=${it.id}">修改</a></td>
					</tr>					
					</c:forEach>
				</table>
			</div>
