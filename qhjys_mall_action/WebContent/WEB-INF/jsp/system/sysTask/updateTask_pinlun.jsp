...
<div class="membercontent">
	...
	<div class="memberright">
          ...
          <p class="clearfix">
        	...
        	<input class="con-email" name="taskFrom" value="${task.taskFrom }" id="taskFrom"/>
          </p>
          <p class="clearfix">
        	<label class="textw100" for="con-email4">公司内部对接人:</label>
        	<input class="con-email" name="staff" value="${task.staff }" id="staff"/>
          </p>
          <p class="clearfix">
        	<label class="textw100" for="con-email4" >审核时间:</label>
        	<input class="con-email" name="verifyTime" value="${task.verifyTime }" id="verifyTime"/> 天
          </p>
          <p class="clearfix">
        	<label class="textw100" for="con-email4" >达标评价星级数:</label>
        	<select  name="appraiseRank" id="appraiseRank">
        		<option value="">请选择...</option>
        		<option value="1" <c:if test="${task.appraiseRank ==1 }">selected="selected"</c:if>>1星</option>
        		<option value="2" <c:if test="${task.appraiseRank ==2 }">selected="selected"</c:if>>2星</option>
        		<option value="3" <c:if test="${task.appraiseRank ==3 }">selected="selected"</c:if>>3星</option>
                <option value="4" <c:if test="${task.appraiseRank ==4 }">selected="selected"</c:if>>4星</option>
        		<option value="5" <c:if test="${task.appraiseRank ==5 }">selected="selected"</c:if>>5星</option>
        	</select>
          </p>
        	<p class="clearfix">
