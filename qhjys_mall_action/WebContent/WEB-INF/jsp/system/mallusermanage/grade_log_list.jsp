...
<script type="text/javascript" src="/js/system/mallusermanage/grade_log_list.js"></script>
...
<div class="membercontent">
	<!----------------左侧----------------------->
 	<jsp:include page="/WEB-INF/jsp/system/mallusermanage/uLeft.jsp" flush="true" ><jsp:param name="position" value="4" /></jsp:include>
    <!--------------右侧------------------>
		<div class="memberright">
     <h3 style="padding-left:10px; margin-bottom:25px;">等级积分操作日志</h3>
      <form id="from" name="form1"  class="zcform1" action="/managermall/systemmall/malluser/gradeLogList.do" method="post">
         ...
         <table width="100%" border="0" cellspacing="0" cellpadding="0">
               ...
               <c:forEach items="${page}" var="c">
                <tr>   
                    <td class="td160">${c.userId}</td>
                    <td class="td160"><fmt:formatDate value="${c.createDate}" pattern="yyyy-MM-dd "/></td>
                    <td class="td160">
           				<c:if test="${c.type==1 }"> 赠送积分</c:if>         	
           				<c:if test="${c.type==2 }"> 兑换商品</c:if>         	
                  </td>
                    <td class="td160">${c.currentIntegral }</td>  
                    <td class="td160">${c.changeIntegral }</td>
                    <td class="td160">${c.changeAfterIntegral }</td>
                    <td class="td160">${c.cexplain}</td>              
                </tr>
                </c:forEach>
              </table>
            </div>
        ...
        <div class="page">
            	...
            	<c:when test="${page.getPages()>6}">
            		<c:forEach var="i" begin="1" end="3">
            			<c:choose><c:when test="${i==page.getPageNum()}"><em class="current">${i}</em></c:when>
            			<c:otherwise><a href="javascript:javascript:previousPage(${i},'from','page')">${i}</a></c:otherwise></c:choose>
            		</c:forEach>
