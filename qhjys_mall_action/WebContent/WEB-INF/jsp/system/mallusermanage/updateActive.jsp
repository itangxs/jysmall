...
<div class="membercontent">
	...
	<div class="memberright">
         <h3 style="padding-left:10px; margin-bottom:25px;">站内消息——发送消息</h3>
            <form id="form1" name="form1" action="/managermall/systemmall/active/updateActive.do" class="zcform1">
            <input type="hidden" name="activeId" value="${active.id }">
           <p class="clearfix">
        	<label class="one" for="con-email4" >起始时间：</label>
        	<input id="begin" name="beginTime" class="easyui-datebox con-time" value="${active.beginTime }">
          </p>
          <br>
           <p class="clearfix">
       <label class="one" for="con-email4" >结束时间：</label>
        	<input id="begin" name="endTime" class="easyui-datebox con-time"  value="${active.endTime }">
           </p>
           <br>
           <p class="clearfix">
       <label class="one" for="con-email4" >通用卷元宝：</label>
        	<input class="con-email4" name="commonCoupon" id="commonCoupon"  value="${active.commonCoupon }">
           </p>
           <br>
           <p class="clearfix">
       <label class="one" for="con-email4" >商家卷元宝：</label>
        	<input class="con-email4" name="sellerCoupon" id="sellerCoupon"  value="${active.sellerCoupon }">
           </p>
           <br>
           <p class="clearfix">
       <label class="one" for="con-email4" >有效期(天)：</label>
        	<input class="con-email4" name="couponValidity" id="couponValidity"  value="${active.couponValidity }">
           </p>
           <br>
           <p class="clearfix">
        <label class="one" for="con-email4" >是否启用：</label>
        	 <select name="enabled" id="enabled">
             <option value="1" <c:if test="{active.enabled == 1}">selected="selected"</c:if>>启用</option>
             <option value="0" <c:if test="{active.enabled == 0}">selected="selected"</c:if>>禁用</option>
            </select>
           </p>
           <br>
           <p class="clearfix">
