...
<script type="text/javascript" src="/js/system/finance/seller_account_record.js"></script>
...
<div class="membercontent">
	<!----------------左侧----------------------->
	<jsp:include page="/WEB-INF/jsp/system/finance/uLeft.jsp" flush="true" ><jsp:param name="val" value="3" /></jsp:include>
    <!--------------右侧------------------>
	<!--------------右侧------------------>
<div class="memberright">
            <h3 style="padding-left:10px; margin-bottom:25px;">资金日志——资金操作日志</h3>
           <form id="from" name="from"  class="zcform1" action="/managermall/systemmall/finance/sellerAccountRecordList.do">
    			<input type="hidden" id="page" name="page" value="${pageNum}">
		        <p class="clearfix">
		        	<label class="one" for="con-email4" >业务类型：</label>
		        	<select name="payType">
            	 		<option value="">全部</option>
            	 		<option value="3"  <c:if test="${payType==3 }">selected="selected"</c:if>>商家充值</option>
            	 		<option value="4"	<c:if test="${payType==4 }">selected="selected"</c:if>>商家提现</option>
            		</select>
        ...
        <table width="100%" border="0" cellspacing="0" cellpadding="0">
                ...
                <c:forEach items="${page}" var="page">
	                <tr>
	                    <td class="td160">${page.sellerName}</td>
	                    <td class="td160"><fmt:formatDate value="${page.createDate}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
	                    <td class="td160">${page.amount}</td>
	                    <c:if test="${page.payType == 3}">
	                    	<td class="td160">商家充值</td>
	                    </c:if>
	                     <c:if test="${page.payType == 4}">
	                    	<td class="td160">商家提现</td>
	                    </c:if>
	                </tr>
                </c:forEach>
              </table>
