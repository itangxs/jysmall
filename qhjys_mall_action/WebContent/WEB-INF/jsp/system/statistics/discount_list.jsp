...
<div class="membercontent">
	...
	<div class="memberright">
           ...
           <form id="from" method="post" action="/managermall/systemmall/statistics/discountList.do">
     ...
     <h3 class="jinrongtitle">查询统计</h3>
        ...
        <table width="100%" border="0" cellspacing="0" cellpadding="0">
               <tr>  
                    ...
                    <th class="td60">会员ID</th>
                    <th class="td100">店铺ID</th>
                    <th class="td100">店铺名称</th>
                    <th class="td70">交易金额</th>
                    <th class="td70">手续费率</th> 
                    <th class="td60">手续费</th> 
                    <th class="td80">到账金额</th>       
                    <th class="td70">支付方式</th> 
                    <th class="td70">消费来源</th>
                    <th class="td70">套现属性</th>
                    <th class="td70">套现设置</th>
                </tr>

                <c:forEach items="${page}" var="s">
                <tr>
                    <td class="td100 tdfont">${s.orderNo }</td>
                    <td class="td80 tdfont"><fmt:formatDate value="${s.payTime }" pattern="yyyy-MM-dd HH:mm"/></td>
                    <td class="td60">${s.openId }</td> 
                    <td class="td60">${s.storeId }</td> 
                    <td class="td100">${s.storeName }</td>  
                    <td class="td70">${s.realPay }</td> 
                    <td class="td70">${s.orderRate }%</td> 
                    <td class="td60">${s.rateFee }</td> 
                    <td class="td80">${s.totamt }</td>
                    <td class="td70">  <c:if test="${s.type==1 || s.type==3 || s.type==5 || s.type==18}">
										微信							   
							   </c:if>
							    <c:if test="${s.type==2 || s.type==6 || s.type==17}">
										支付宝						   
							   </c:if>
							    <c:if test="${s.type==7}">
										QQ钱包							   
                    ...
                    <td class="td70"><c:if test="${s.bankType == 1}">零钱</c:if>
                     ...
                     <td class="td80"> <c:if test="${s.isCash == 0}">否</c:if>
					                    <c:if test="${s.isCash == 1}">是</c:if></td>
