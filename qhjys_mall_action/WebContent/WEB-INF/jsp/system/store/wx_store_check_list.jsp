...
<div class="membercontent">
	<!----------------左侧----------------------->
	<jsp:include page="/WEB-INF/jsp/system/store/uLeft.jsp"><jsp:param name="position"  value="5"/></jsp:include>
	<!-- end -->
		<div class="memberright">
             <h3 style="padding-left:10px; margin-bottom:25px;">商家管理——微信店铺列表</h3>
        <form id="from"  class="zcform1" method="post" action="/managermall/systemmall/store/wxChecklist.do">
         <input id="page" name="pageNum" type="hidden" value="${pageNum}">
           ...
           <p class="clearfix">
        	<label class="one" for="con-email4" >修改时间：</label>
              ...
              <c:forEach items="${page}" var="s">
                <tr>
                    ...
                    <td class="td160">
                    	...
                    	<c:if test="${s.status == 2  }">审核不通过</c:if>
                    </td>
                    <td class="td160"><fmt:formatDate value="${s.createTime}" pattern="yyyy-MM-dd HH:mm"/></td> 
                   
                    <td class="td160">
                    <button onclick="javascript:window.location.href='/managermall/systemmall/store/wxStoreCheck.do?id=${s.id}'">查看/审核 </button>
