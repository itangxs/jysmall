...
function gehang(){
  ...
  function updateStatus(status){//修改状态
    	   var input=document.getElementsByName("ids");
	       	...
	       	if(ids.length>0){
	       		if(confirm("id="+ids+"修改状态?")){
	       			var urlval ="updateSellerEnabled.do";
  ...
  function updateResetSellerPassWord(id){//删除
      		if(confirm("确定要重置密码吗?")){
      			var urlval ="updateResetSellerPassWord.do";
      			var parma ="id="+id;
	       		   $.ajax({
	   		    	type:'post',//可选get
	   		    	url:urlval,//这里是接收数据的后台程序
