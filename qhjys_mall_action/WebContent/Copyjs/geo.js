...
function change(v) {
	var str = "0";
	for (i = 0; i < v; i++)
		str += ("_" + (document.getElementById(els[i]).selectedIndex - 1));
	var ss = document.getElementById(els[v]);
	with (ss) {
		length = 0;
		options[0] = new Option(opt0[v], "");
		if (v && document.getElementById(els[v - 1]).selectedIndex > 0 || !v) {
			if (dsy.Exists(str)) {
				ar = dsy.Items[str];
...
function preselect(p_key) {
    ...
    if (index < provinces.length) {
        document.getElementById(els[0]).selectedIndex = index + 1;
        change(1);
    }
}
...
var opt0 = ["省份", "地级市", "市、县、区"];
function setup(els) {
    for (i = 0; i < els.length - 1; i++)
        document.getElementById(els[i]).onchange = new Function("change(" + (i + 1) + ");");
    change(0);
}