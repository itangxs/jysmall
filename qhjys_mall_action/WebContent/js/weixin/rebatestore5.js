	...
	var orderNo = res.orderNo;
	 WeixinJSBridge.invoke('getBrandWCPayRequest',
		...
		if(rs.err_msg == "get_brand_wcpay_request:cancel"){
			...
			location.href = '/wxMall/paySuccessPage.do?orderId='+orderId;
		}else{
...
function storeList(pageNum){
	var path = $("#path").val();
	$.ajax({
		url: path+"/wxMall/rebateStoreList.do",
		type: "POST",
				...
				if(result.data.page == null ){
					...
					$.each(result.data.page,function(index,item){
						$("#storeList").append("<li> <div class=\"proimg\"> "+
								"<a href=\"javascript:toTurnRebateDetail("+item.rebateId+")\"><img src=\""+path+item.logo+"\" ></a></div><div class=\"centerinfo\">"+
...
function payOrder(){
	...
	var channelValidation =$("#channelValidation").val(); //店铺通道识别 1为民生通道
	alert("...");
	alert(channelValidation);
	var realpay; //实际支付金额
...
//		noDiscount = 0;
//	}
	var data = "totamt="+totamt+"&noDiscount="+noDiscount+"&rebate="+rebate+"&rebateId="
	           +rebateId+"&sellerId="+sellerId+"&storeId="+storeId+"&storeName="+storeName+"&realPay="+realpay+"&needPay="+realpay+"&openId="+openId+"&headimgurl="+headimgurl+"&nickname="+nickname;
	...
	if (payType == 0) {
		...
		if(channelValidation == 1){
			$.ajax({
				url: "/wxMall/toPayOrderAlipayMS.do",
				type: "POST",
				data: data,
				success: function(result){
		...
		if(channelValidation == 3){
			$.ajax({
				url: "/wechatPay/toPayOrderXYEQAlipay.do",
				type: "POST",
				data: data,
				success: function(result){
					var orderNo = result.orderNo;
		 ...
		 if($(".payquan")){  //判断是否存在卡券  
			 var check =  $("input[name='realPayCheck']").is(':checked');
			 if(check){//是否勾选卡券
				 var couponId =  $("input[name='realPayCheck']").attr("id").split("_")[1];
				 realpay = $("input[name='realPayCheck']").attr("value");
				 var needPay = totamt;
				 totamt = realpay;
				 data ="couponId="+couponId+"&totamt="+totamt+"&noDiscount="+noDiscount+"&rebate="+rebate+"&rebateId="
					+rebateId+"&sellerId="+sellerId+"&storeId="+storeId+"&storeName="+storeName+"&realPay="+realpay+"&needPay="+needPay+"&openId="+openId+"&headimgurl="+headimgurl+"&nickname="+nickname;
			 }
		 }
		 if(channelValidation == 1){
			$.ajax({
				url: "/wxMall/toPayOrderWXMS.do",
				type: "POST",
				data: data,
				success: function(result){
					if(result.code == "0"){
						...
						alert("系统繁忙请稍后再试！");
					}else if(result.code == "00009527"){ //0元支付成功
						location.href = '/wxMall/paySuccessPage.do?orderId='+result.orderId;
					}
				},
		 ...
		 if(channelValidation == 3){
				$.ajax({
					url: "/wechatPay/toPayOrderXYEQWX.do",
					type: "POST",
					data: data,
					success: function(result){
						if(result.code == "0"){
